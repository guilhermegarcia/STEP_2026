...
q <- read_csv2("data/survey_2026.csv")
...
names(q)
q <- q |>
  select(-matches("^(heure|nom|adresse|total|quiz|feedback|points)"))
...
long <- long |>
  ...
  mutate(
    across(3:8, ~ factor(.x, ordered = TRUE))
  )

...
ggplot(data = d, aes(x = Affix |> fct_reorder(RT, .fun = mean), y = RT)) +
  stat_summary(
    geom = "line", color = "black", alpha = 0.2,
...
ggplot(
  data = d |> filter(
    Participant %in% c("Part_5", "Part_10", "Part_15")
  ),
...
# NOTE: This is a good time to check plotly
ggplotly()

# NOTE: Or we could check out stems
# and provide a sample for the figure:
d <- d |>
  mutate(Stem = str_sub(Word, start = 1, end = -4L))

set.seed(6)
ggplot(
  data = d,
  aes(x = Affix, y = RT, label = Stem)
) +
  ...
  geom_text(data = d |> sample_n(size = 30), alpha = 0.2)
